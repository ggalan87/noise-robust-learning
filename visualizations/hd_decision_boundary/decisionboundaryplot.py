...
class DBPlot(BaseEstimator):
    ...
    def __init__(
        ...
        self.penalties_enabled = False  # True
    ...
    def _prepare_fit(self, X, y, training_indices):
    ...
    def _step2(self, X, y, training_indices):
        """
        step 2. look for decision boundary points on lines connecting randomly sampled points of majority &
        minority class

        params same as in fit
        """
    ...
    def _step3(self):
        """
        step 3. look for decision boundary points between already known db points that are too distant
        (search on connecting line first, then on surrounding hypersphere surfaces)
        """
    ...
    def fit(self, X, y, training_indices=None):
        ...
        self._prepare_fit(X, y, training_indices)

        print('Step 1')
        self._step1()

        print('Step 2')
        self._step2(X, y, training_indices)
    ...
    def plot(
        ...
        plt.scatter(
            ...
            edgecolor=[
                "g"
                if self.y_pred[self.train_idx[i]] == 1
                else "r"
            ...
            for j in range(n):
                ...
                for k in range(retries):
                    ...
                    if (
                        ...
                        if (
                                self.X2d_xmin <= db_point2d[0] <= self.X2d_xmax
                                and self.X2d_ymin <= db_point2d[1] <= self.X2d_ymax
                        ):
                            self.decision_boundary_points.append(db_point)
                            self.decision_boundary_points_2d.append(db_point2d)
    ...
    def _find_decision_boundary_along_line(
        ...
        def objective(l, grad=0):
            # interpolate between source and destination; calculate distance from
            # decision boundary
