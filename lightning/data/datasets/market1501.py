...
class Market1501DatasetPart(IdentityImageDataset):
    ...
    def __init__(
        ...
        super(Market1501DatasetPart, self).__init__(root,
                                                    part_name,
                                                    transform=transform,
        ...
        variants = \
            [
                ...
                'small_cluster_noise_0.25_nn',
                'small_cluster_noise_0.5_nn',
    ...
    def _load_data(self):
        # Modified from:
        # https://github.com/KaiyangZhou/deep-person-reid/blob/master/torchreid/data/datasets/image/market1501.py
    ...
    def raw_folder(self) -> Path:
        return Path(self.root) / f'market1501' / 'Market-1501-v15.09.15'


def pass_dataset():
    market_train = Market1501DatasetPart('/data/datasets/', part_name='train', relabel=True,
                                         training_variant='small_cluster_noise_0.5')

    noisy = 0
    non_noisy = 0
    for entry in market_train.data:
        if entry['is_noisy']:
            noisy += 1
        else:
            non_noisy += 1
...
def generate_noisy_labels():
    ...
    market_train = Market1501DatasetPart('/data/datasets/', part_name='train', relabel=True, training_variant=None)

    market_train_0_5_symmetric = (
        disturb_records_targets(market_train.data, labels_noise_perc={'symmetric': 0.5}, key_to_disturb='target'))
    market_train_0_2_symmetric = (
        disturb_records_targets(market_train.data, labels_noise_perc={'symmetric': 0.2}, key_to_disturb='target'))
    market_train_0_1_symmetric = (
        disturb_records_targets(market_train.data, labels_noise_perc={'symmetric': 0.1}, key_to_disturb='target'))

    noisy_labels_root = Path('/media/amidemo/Data/object_classifier_data/datasets/market1501/noisy_labels/')
    save_noisy_data(market_train_0_1_symmetric, noisy_labels_root / 'symmetric_noise_0.1.pkl')
    save_noisy_data(market_train_0_2_symmetric, noisy_labels_root / 'symmetric_noise_0.2.pkl')
    save_noisy_data(market_train_0_5_symmetric, noisy_labels_root /'symmetric_noise_0.5.pkl')
