...
from lightning.data.datasets import DukeMTMCreIDDatasetPart

from lightning.data.data_modules import patch_visiondatamodule


@dataclass
class DukeMTMCreIDArgs:
    training_variant: Optional[str] = None
    ignore_noisy_samples: bool = False


class DukeMTMCreIDDataModule(IdentityDataModule):
    name: str = "dukemtmcreid"
    dataset_cls = DukeMTMCreIDDatasetPart
    # dims = (3, 128, 64)
    ...
    def __init__(
            ...
            dataset_args: DukeMTMCreIDArgs = None,
            *args: Any,
            **kwargs: Any,
    ) -> None:
        super().__init__(
...
def pass_dataloader():
    ...
    dm = DukeMTMCreIDDataModule(data_dir="/media/amidemo/Data/object_classifier_data/datasets",
                              batch_size=64,
                              train_transforms=SoliderTrainTransforms(),
                              val_transforms=SoliderTransforms(),
                              test_transforms=SoliderTransforms(),
                              val_split=0.0,
                              dataset_args=DukeMTMCreIDArgs(training_variant='symmetric_noise_0.5'))
    dm.setup('fit')
